...
class Camera1 extends CameraViewImpl {
    ...
    Set<AspectRatio> getSupportedAspectRatios() {
        Set<AspectRatio> idealAspectRatios = new ArraySet<>();

        for (AspectRatio aspectRatio : mPreviewSizes.ratios()) {
            if (mPictureSizes.sizes(aspectRatio) != null)
                idealAspectRatios.add(aspectRatio);
        }
    ...
    boolean setAspectRatio(AspectRatio ratio) {
        if (mAspectRatio == null || !isCameraOpened()) {
            ...
            if (sizes == null) {
                Log.e("Size", ratio + " is not supported");
...
//                    "Camera is not ready. Call start() before takePicture().");
    ...
    private void openCamera() {
        ...
        if (mAspectRatio == null) {
            ...
            if (ratios.size() > 0) {
                mAspectRatio = (AspectRatio) ratios.toArray()[ratios.size() - 1];
            } else {
...
//        if (sizes == null) { // Not supported
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP || mFacing == CameraView.FACING_BACK) {
            Set<AspectRatio> ratios = getSupportedAspectRatios();
            if (ratios.size() > 0) {
                mAspectRatio = (AspectRatio) ratios.toArray()[ratios.size() - 1];
            } else {
...
//    }
    ...
    void setZoom(boolean zoomInOrOut) {
        if (isCameraOpened()) {
            try {
                if (mCameraParameters.isZoomSupported()) {
                    int MAX_ZOOM = mCameraParameters.getMaxZoom();
                    ...
                    Log.e("Camera1", "Zoom is not available");
                }
            } catch (Exception e) {
