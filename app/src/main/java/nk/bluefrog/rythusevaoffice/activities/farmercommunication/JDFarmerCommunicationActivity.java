...
public class JDFarmerCommunicationActivity extends BluefrogActivity implements SearchableSpinner.SpinnerListener, ResponseListener, SearchableMultiSpinner.SpinnerListener, AdapterView.OnItemSelectedListener {

    final public int GPS_FLAG = 1245;
    private final int galleryRequestCode = 456;
    private DBHelper dbHelper;
    private SearchableMultiSpinner spinnerVillage, spinnerMandal;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.jd_farmer_communication);
        dbHelper = new DBHelper(this);
        setToolBar(getString(R.string.farmer_communication), getString(R.string.sub_title_form));
        initViews();
    }
    ...
    private void initViews() {
        ...
        List<SpinnerData> mandalData = getMandalData();
        spinnerMandal.setItems(mandalData, this);
        spinnerMandal.setTag(mandalData);
    ...
    private void getVillageBasedOnManadal(List<String> selectedIds) {

        if (selectedIds.size() == 1 && selectedIds.get(0).equals("0")) {
            List<SpinnerData> villageData = getVillageData();
            spinnerVillage.setItems(villageData);
            spinnerVillage.setTag(villageData);

        } else {
            List<SpinnerData> villageData = getVillageBasedOnIdSelection(selectedIds);
            spinnerVillage.setItems(villageData);
            spinnerVillage.setTag(villageData);
        }
    ...
    private List<SpinnerData> getVillageBasedOnIdSelection(List<String> mandals) {
        ...
        List<List<String>> gpData = dbHelper.getGpBasedonIdSelection(dbHelper.getSplitMandalId(mandals));
        if (gpData.size() > 0) {
            for (int i = 0; i < gpData.size(); i++) {
                data = new SpinnerData();
                data.setId(gpData.get(i).get(0));
                if (spinnerMandal.getSelectedIds().size() == 1) {
                    data.setName(gpData.get(i).get(1));
                } else {
                    data.setName(gpData.get(i).get(1) + " (" + gpData.get(i).get(2) + ")");
                }
                mandalData.add(data);
    ...
    private String getIds(List<SpinnerData> spinnerData) {
        StringBuilder data = new StringBuilder();
        for (int i = 0; i < spinnerData.size(); i++) {
            if (i == 0) {
                data.append(spinnerData.get(i).getId());
            } else {
                data.append(",").append(spinnerData.get(i).getId());
            }
        ...
        if (checkValidation()) {
            ...
            if (Helper.isNetworkAvailable(getApplicationContext())) {
                ...
                try {
                    ...
                    for (int i = 0; i < spinnerMandal.getSelectedItems().size(); i++) {
                        if (i == 0) {
                            if (spinnerMandal.getSelectedItems().get(i).getId().trim().equals("0")) {
                                mandalids = new StringBuilder(getIds((List<SpinnerData>) spinnerMandal.getTag()));
                            } else {
                                mandalids = new StringBuilder(spinnerMandal.getSelectedItems().get(i).getId());
                            }

                        } else {
                            mandalids.append(",").append(spinnerMandal.getSelectedItems().get(i).getId());
                        }
                    ...
                    for (int i = 0; i < spinnerVillage.getSelectedItems().size(); i++) {
                        ...
                        if (i == 0) {
                            if (spinnerVillage.getSelectedItems().get(i).getId().trim().equals("0")) {
                                villageIds = new StringBuilder(getIds((List<SpinnerData>) spinnerVillage.getTag()));
                            } else {
                    ...
                    jobMain.put("mandals", mandalids.toString());
        ...
        askCompactPermissions(new String[]{Manifest.permission.CAMERA, Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE},
                new PermissionResult() {
                    ...
                    public void permissionDenied() {
                        Helper.showToast(JDFarmerCommunicationActivity.this, getString(R.string.accept_cam_permissions));
                        //setCancelResult();
                    }

                    @Override
                    public void permissionForeverDenied() {
                        openSettingsApp(JDFarmerCommunicationActivity.this);
                    }
                });
    ...
    public void onSuccess(int responseCode, String strResponse) {
        ...
        try {
            ...
            if (jobj.getString("Status").contains("$200")) {
                ...
                insertIntoDB();
                Helper.showToast(JDFarmerCommunicationActivity.this, getString(R.string.not_submit));
            }
    ...
    public void onError(int responseCode, String error) {
        Helper.showToast(JDFarmerCommunicationActivity.this, error);
        insertIntoDB();
    }
    ...
    private void insertIntoDB() {
        ...
        dbHelper.insertintoTable(DBTables.FarmerCommunication.TABLE_NAME, DBTables.FarmerCommunication.cols,
                new String[]{gpIds.toString(), gpNames.toString(), spinnerCrop.getSelectedId().get(0), etTitle.getText().toString(), etDescription.getText().toString(), "1"
                        ...
                        , "", "", getMandalIds(spinnerMandal.getSelectedIds()), getMandNames(spinnerMandal.getSelectedItems())});
       ...
       /* dbHelper.insertintoTable(DBTables.FarmerCommunication.TABLE_NAME, DBTables.FarmerCommunication.cols,
        ...
        Helper.showToast(JDFarmerCommunicationActivity.this, getString(R.string.submit_suceess));
        finish();
    }
    ...
    public void onItemsSelected(View view, List<SpinnerData> list, List<SpinnerData> list1) {
        ...
        if (view == spinnerMandal) {
            if (list1.size() > 0) {
                getVillageBasedOnManadal(spinnerMandal.getSelectedIds());
            }
        } else if (view == spinnerVillage) {
            if (list.size() > 0) {
