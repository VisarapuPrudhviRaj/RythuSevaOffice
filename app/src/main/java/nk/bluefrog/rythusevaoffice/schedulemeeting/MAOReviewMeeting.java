...
public class MAOReviewMeeting extends BluefrogActivity implements SearchableSpinner.SpinnerListener
    ...
    private SearchableSpinner villageSpinner, sp_mandal;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_mao_review_meeting);
        dbHelper = new DBHelper(this);
        setToolBar(getString(R.string.mao_meeting_review), getIntent().getStringExtra("date"));
        initViews();
    ...
    private void initViews() {
        ...
        villageSpinner.setItems(new ArrayList<SpinnerData>(), this);
    ...
    public void submitReview(View view) {
        ...
        if (checkValidation()) {
            ...
            if (Helper.isNetworkAvailable(getApplicationContext())) {
                ...
                try {
                    jobMain.put("MPEO_MobileNo", PrefManger.getSharedPreferencesString(this, Constants.sp_mobile, ""));
                    /*Key need to add*/
                    jobMain.put("MandalId", sp_mandal.getSelectedId());
                    jobMain.put("GPID", villageSpinner.getSelectedId());
    ...
    public void onSuccess(int responseCode, String strResponse) {
        ...
        try {
            ...
            if (jobj.getString("status").contains("$200")) {
                ...
                Helper.showToast(MAOReviewMeeting.this, getString(R.string.submit_suceess));
                finish();
            } else {
                Helper.showToast(MAOReviewMeeting.this, getString(R.string.not_submit));
            }
    ...
    public void onError(int responseCode, String error) {
        ...
        Helper.showToast(MAOReviewMeeting.this, error);
    ...
    public void onItemsSelected(View view, List<SpinnerData> list, int i) {
        ...
        if (view == sp_mandal) {
            if (i != -1) {
                getVillageBasedOnMandal(sp_mandal.getSelectedItem().get(0).getId());
            }
        } else if (view == villageSpinner) {
            if (i != -1) {
                loadFarmers(villageSpinner.getSelectedItem().get(0).getId());
            }
        }
    ...
    private void loadFarmers(String id) {
        int count = dbHelper.getCountByValues(DBTables.MeetingReview.TABLE_NAME, new String[]{DBTables.MeetingReview.MEETING_DATE
                ...
                , new String[]{getIntent().getStringExtra("date"), id});
        ...
        farmerData = dbHelper.getTableColDataByCond(DBTables.FarmerTable.TABLE_NAME
                , DBTables.FarmerTable.NAME + "," + DBTables.FarmerTable.AADHAR + "," + DBTables.FarmerTable.MOBILE
                , new String[]{DBTables.FarmerTable.GP_ID}, new String[]{id});
    ...
    private void getVillageBasedOnMandal(String id) {
        villageSpinner.setItems(getVillageBasedOnIdSelection(id));
    }
        ...
        askCompactPermissions(new String[]{Manifest.permission.CAMERA, Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE},
                new PermissionResult() {
                    ...
                    public void permissionDenied() {
                        Helper.showToast(MAOReviewMeeting.this, getString(R.string.accept_cam_permissions));
                        //setCancelResult();
                    }

                    @Override
                    public void permissionForeverDenied() {
                        openSettingsApp(MAOReviewMeeting.this);
                    }
                });
